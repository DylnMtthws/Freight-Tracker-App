...
function NotDeliveredTable({ orders }) {
  ...
  const notDeliveredOrders = orders.filter(
    (order) =>
      ...
      order["Scheduled Date"] !== "12/30/2049 23:00:00" &&
  ...
  const currentItems = notDeliveredOrders.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const notDeliveredOrdersDisplay = sortedItems.map((order) => (
  ...
  const totalPages = Math.ceil(notDeliveredOrders.length / itemsPerPage);
