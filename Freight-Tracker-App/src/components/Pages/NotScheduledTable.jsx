...
function NotScheduledTable({ orders }) {
  const notScheduledOrders = orders.filter(
    (order) =>
  ...
  const currentItems = notScheduledOrders.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const notScheduledOrdersDisplay = sortedItems.map((order) => (
  ...
  const totalPages = Math.ceil(notScheduledOrders.length / itemsPerPage);
